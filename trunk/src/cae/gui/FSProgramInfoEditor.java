...
/**
 * Editor de archivos cae.
 * 
 * @author caeycae
 *
 */
public class FSProgramInfoEditor extends JDialog
{
	private static final Logger LOGGER = Logger.getLogger(FSProgramInfoEditor.class);
	...
	public FSProgramInfoEditor(String title, FSProgramInfo info)
	{
		this.setTitle(title);
		...
		osCombo = new JComboBox<String>(FSServices.getSO());
		cp.add(osCombo, c);
		bitsCombo = new JComboBox<String>(FSServices.getBits());
		cp.add(bitsCombo, c);
		categoryCombo= new JComboBox<String>(FSServices.getCategory());
		...
		exeButton.addActionListener(new FSFileFinderAction());
		cp.add(exeButton, c);
		c.gridx = 2;
		c.gridy = GridBagConstraints.RELATIVE;
		keygenButton = new JButton("...");
		keygenButton.addActionListener(new FSFileFinderAction());
		cp.add(keygenButton, c);
		patchButton = new JButton("...");
		patchButton.addActionListener(new FSFileFinderAction());
		cp.add(patchButton, c);
		leameButton = new JButton("...");
		leameButton.addActionListener(new FSFileFinderAction());
		...
		okButton.addActionListener(new FSResponseAction());
		subPanel.add(okButton);
		cancelButton = new JButton("Cancelar");
		cancelButton.addActionListener(new FSResponseAction());
		subPanel.add(cancelButton);

		panel.add(subPanel, BorderLayout.EAST);
		cp.add(panel, c);

		// TODO --- para hacer shortcuts
		//setMnemonic
	...
	protected void saveData() throws FileNotFoundException, IOException {
		...
		if(exeTextField.getText() != null && !exeTextField.getText().isEmpty())
			info.setExe(exeTextField.getText());
		else
			info.setExe(null);

		if(keygenTextField.getText() != null && !keygenTextField.getText().isEmpty())
			info.setKeygen(keygenTextField.getText());
		else
			info.setKeygen(null);

		if(patchTextField.getText() != null && !patchTextField.getText().isEmpty())
			info.setPatch(patchTextField.getText());
		else
			info.setPatch(null);

		if(leameTextField.getText() != null && !leameTextField.getText().isEmpty())
			info.setLeame(leameTextField.getText());
		else
			info.setLeame(null);

		FSProgramInfoFileAccess.write(info);
	}

	private void updateData(FSProgramInfo info) {
		...
		if(info.getExe() != null)
			exeTextField.setText(info.getExe());
		if(info.getKeygen() != null)
			keygenTextField.setText(info.getKeygen());
		if(info.getPatch() != null)
			patchTextField.setText(info.getPatch());
		if(info.getLeame() != null)
			leameTextField.setText(info.getLeame());
	}
	...
	public FSProgramInfo getInfo() {
		return info;
	}


	class FSFileFinderAction implements ActionListener
	{
		@Override
	...
	class FSResponseAction implements ActionListener
	{
		public void actionPerformed(ActionEvent e) {
